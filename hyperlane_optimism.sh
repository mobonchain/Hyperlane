...
VALIDATOR_DIR="/opt/hyperlane_db_optimism/$VALIDATOR_NAME"
...
docker run -d \
    --name "$CONTAINER_NAME" \
    --mount type=bind,source="$VALIDATOR_DIR",target=/hyperlane_db_optimism \
    "$IMAGE_NAME" \
    ./validator \
    --db /hyperlane_db_optimism \
    --originChainName optimism \
    --reorgPeriod 1 \
    --validator.id "$VALIDATOR_NAME" \
    --checkpointSyncer.type localStorage \
    --checkpointSyncer.folder optimism \
    --checkpointSyncer.path /hyperlane_db_optimism/optimism_checkpoints \
    --validator.key "$PRIVATE_KEY" \
    --chains.optimism.signer.key "$PRIVATE_KEY" \
    --chains.optimism.customRpcUrls "$RPC_URL"
