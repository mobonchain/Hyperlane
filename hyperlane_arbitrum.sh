...
VALIDATOR_DIR="/opt/hyperlane_db_arbitrum/$VALIDATOR_NAME"
...
docker run -d \
    --name "$CONTAINER_NAME" \
    --mount type=bind,source="$VALIDATOR_DIR",target=/hyperlane_db_arbitrum \
    "$IMAGE_NAME" \
    ./validator \
    ...
    --chains.arbitrum.signer.key "$PRIVATE_KEY" \
